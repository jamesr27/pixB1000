...
MotorController::run_controller(float dt)
 {
	 ...
	 else if (_rc_channels.channels[6] <= 0.3f && _rc_channels.channels[6] >= -0.5f)
	 {
		 _switch_state = 1;
	 }
	 else if (_rc_channels.channels[6] > 0.3f)
	 {
		 _switch_state = 2;
	 }
...
//	 }
	 ...
	 if (_rc_channels.channels[7] > 0.0f && _rc_channels.channels[7] > 0.0f)
	 {
		_motor_kill.kill_switch = true;
	 }
	 ...
	 if (_actuator_armed.armed)
	 {
		 ...
		 else if (_params.motor_control_bypass == 1)
		 {
			 ...
			 {
				_motor_throttle.throttle = _rc_channels.channels[6]/2.0f + 0.5f + _idle_offset;
			 }
		 }
